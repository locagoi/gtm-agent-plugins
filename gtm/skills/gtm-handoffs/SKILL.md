...
- **The enroll column is the LAST column, never a middle one.** It sends only in its own run and never on create. `auto_run: true` on it does not go through: the call is refused with a demand for a `run_condition` **that is refused again even when you supply one**, at the top level or inside `config` (tested on a live workspace). So auto-enrollment is unreachable by this path and enrollment stays a deliberate act — but be aware the message misdescribes the reason, and do not spend an afternoon trying to satisfy it. The platform's own legend still claims a flat rejection; the behaviour is the refusal-with-wrong-reason above.
