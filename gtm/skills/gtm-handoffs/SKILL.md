...
- **The enroll column is the LAST column, never a middle one.** It sends only in its own run and never on create. **Auto-enrollment is opt-in, and it requires a gate.** `auto_run: true` on the enroll column is accepted only together with a `run_condition` that says WHO gets enrolled automatically — without one it is refused, because the cascade would otherwise enroll every row whose data happens to be complete. Leave it off unless you have watched the qualification get it right on real rows. Note the platform's own handoff legend may still describe a flat rejection of `auto_run`; the behaviour above is what the tool does.
